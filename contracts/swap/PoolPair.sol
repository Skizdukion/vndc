...
contract PoolPair is IPoolPair, LiquidityTokenERC20 {
    ...
    function setSwapFee(uint32 _swapFee) external {
        require(_swapFee > 0, "PoolPair: lower then 0");
        require(msg.sender == factory, "PoolPair: FORBIDDEN");
        require(_swapFee <= 10000, "PoolPair: FORBIDDEN_FEE");
        swapFee = _swapFee;
    }

    function setDevFee(uint32 _devFee) external {
        require(_devFee > 0, "PoolPair: lower then 0");
        require(msg.sender == factory, "PoolPair: FORBIDDEN");
        require(_devFee <= 500, "PoolPair: FORBIDDEN_FEE");
        devFee = _devFee;
    }
    ...
    function swap(
        ...
        uint256 balance1;
        {
            ...
            if (data.length > 0)
                ICallee(to).WineryCall(msg.sender, amount0Out, amount1Out, data);
            balance0 = IERC20(_token0).balanceOf(address(this));
            balance1 = IERC20(_token1).balanceOf(address(this));
        }
